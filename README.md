...
## バージョン管理（`PBPM_APP_VERSION`）

### 意図

- **版の正はリポジトリ内の 1 か所だけ**にし、画面・手順書・問い合わせで「いま動いているのが何版か」を一致させる。  
- 各所に手書きで番号を書かない（更新漏れ・旧版との混在を防ぐ）。  
- 可能なら **Git のタグ**（例: `v1.2.0`）と同じ番号にそろえ、GitHub 上で「その時点の公式リリース」を追いやすくする。

### 版の定義場所

- `OmniStep.js` 先頭付近の **`PBPM_APP_VERSION`**（例: `"1.2.0"`）。リリースのたびに **ここだけ**上げる。  
- アプリ内では **⚙ 設定**モーダルに同じ値が表示される。

### 番号の付け方（SemVer：3 桁）

形式は **`MAJOR.MINOR.PATCH`**（例: `1.2.0`）。4 桁目は付けない（標準のセマンティックバージョニングに合わせる）。

| 桁 | 上げるタイミング（目安） |
|----|---------------------------|
| **MAJOR（1 桁目）** | **互換性を壊す変更**（既存データや使い方がそのままでは困る、移行が必要、など） |
| **MINOR（2 桁目）** | **互換を保った機能・体験の更新**（新機能、見た目・操作の追加や変更で、従来のデータはそのまま使える） |
| **PATCH（3 桁目）** | **小さな修正・内部改善**（バグ修正、利用者から見て挙動・画面が実質同じ、など） |

迷ったら **PATCH を上げる**でよい。影響が大きいときだけ MINOR / MAJOR を検討する。

### リリース時の作業（目安）

1. `PBPM_APP_VERSION` を上げてコミットする。  
2. 必要なら Git で **`git tag vX.Y.Z`**（例: `v1.2.0`）を付けて push する。  
3. GitHub Pages 利用時は、いつもどおり `git push` で反映する。
